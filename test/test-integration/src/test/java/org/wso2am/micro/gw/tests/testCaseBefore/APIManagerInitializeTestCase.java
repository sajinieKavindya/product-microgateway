...
public class APIManagerInitializeTestCase extends APIMLifecycleBaseTest {
    ...
    private Boolean checkForAPIMServerStartup() throws IOException {
        HttpResponse response = HttpClientRequest.doGet(URLs.getAPIMServiceURLHttp("/services/Version"));
        return Objects.nonNull(response) && response.getResponseCode() == 200;
    }

//    private HttpResponse waitForAPIMServerStartup() throws InterruptedException, IOException {
//        int initialWaitingTime = 120 * 1000;
//        int intermediateWaitingTime = 20 * 1000;
//
//        // wait for a constant time until the apim server is started
//        Thread.sleep(initialWaitingTime);
//
//        int retries = 0;
//        int maxRetries = (60 * 1000 * 2) / intermediateWaitingTime;
//        HttpResponse response;
//        do {
//            response = HttpClientRequest.doGet(URLs.getAPIMServiceURLHttp("/services/Version"));
//            if (Objects.nonNull(response) && response.getResponseCode() == 200) {
//                break;
//            }
//            // wait for 20 seconds until the next health check
//            Thread.sleep(intermediateWaitingTime);
//            retries++;
//        }
